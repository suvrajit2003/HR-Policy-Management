...
//     // 2. Get salary data with components
...
//       const leaveDays = Math.floor((overlapEnd - overlapStart) / (1000 * 60 * 60 * 24)) + 1;
...
//     // 6. Calculate present days
//     const presentDays = workingDays - (paidLeaves + unpaidLeaves);

//     // 7. Calculate daily rate from BASE SALARY (not gross salary)
//     const baseSalary = salaryRecord ? salaryRecord.basicSalary : employee.salary;
//     const dailyRate = baseSalary / workingDays;
//     const deductionAmount = Math.round(unpaidLeaves * dailyRate * 100) / 100;

//     // 8. Calculate gross and net salary
//     const grossSalary = salaryRecord ? salaryRecord.grossSalary : employee.salary;
//     const netSalary = Math.round((grossSalary - deductionAmount) * 100) / 100;
...
//     // 10. Add components if salary record exists
//     if (salaryRecord) {
//       response.earnings = salaryRecord.components.filter(c => !c.isDeduction);
//       response.deductions = salaryRecord.components.filter(c => c.isDeduction);
//     }
...
// Update the generate-salary-slip endpoint
// In server.js, find and replace the entire /generate-salary-slip route
